...
//
// Un seul lien par facture, gardé d'une version à l'autre : celui que le
// client a déjà reçu montre toujours la version à jour. Une facture corrigée
// remplace donc vraiment la précédente, sans rien avoir à reprendre.
...
export async function publishShare(doc, settings) {
  const db = cloud()
  if (!db) throw new Error("La sauvegarde infonuagique n'est pas configurée.")
  const user = await requireUser(db)

  // le jeton déjà attribué à cette facture a la priorité, d'où qu'il vienne :
  // un lien envoyé au client ne doit jamais cesser de fonctionner
  const { data: existing } = await db.from('shares').select('token').eq('doc_id', doc.id).maybeSingle()
  const token = existing?.token || doc.shareToken || newToken()
...
// Ce que l'app garde en mémoire pour chaque facture partagée :
// { [docId]: { token, revoked, revision, views, last: {at, revision, kind, id} } }
...
export async function pullShareActivity() {
  ...
  const [{ data: shares, error: e1 }, { data: views, error: e2 }] = await Promise.all([
    db.from('shares').select('token, doc_id, revision, revoked, updated_at'),
  ...
  for (const s of shares || []) {
    byToken.set(s.token, s.doc_id)
    state[s.doc_id] = { token: s.token, revoked: !!s.revoked, revision: Number(s.revision || 1), views: 0, last: null }
  }
  // les ouvertures arrivent de la plus récente à la plus ancienne
  for (const v of views || []) {
    const docId = byToken.get(v.token)
    if (!docId || !state[docId]) continue
    const st = state[docId]
    st.views += 1
...
// Les ouvertures survenues depuis le dernier coup d'œil de l'utilisateur.
export function newViews(state, docs, seenId) {
  const out = []
  for (const doc of docs || []) {
    const st = state?.[doc.id]
    if (!st?.last || Number(st.last.id) <= Number(seenId || 0)) continue
    out.push({ docId: doc.id, number: doc.number, client: doc.client?.name || '', ...st.last })
  }
