...
const ACCENTS = ['#2e7d32', '#1565c0', '#0f172a', '#b45309', '#7b1fa2', '#c62828']

export function SettingsScreen({ settings, setSettings }) {
  ...
  return <section className="screen">
    <header className="screen-head">
      <div><h1>Settings</h1><p className="muted">Ces infos sortent automatiquement sur chaque PDF.</p></div>
    </header>
    ...
    <div className="grid two">
      <div className="card">
        ...
        {settings.logo && <div className="logo-preview">
          ...
          <button className="soft" onClick={() => setSettings({ ...settings, logo: '' })}>Retirer logo</button>
        </div>}
      </div>
      ...
      <div className="card">
        ...
        <label className="check">
          ...
          Charger la taxe par défaut sur les nouvelles invoices
        </label>
        <div className="grid two smallgap">
          <Field label="Préfixe invoices"><input value={settings.invoicePrefix} onChange={e => setSettings({ ...settings, invoicePrefix: e.target.value })}/></Field>
          <Field label="Préfixe estimates"><input value={settings.estimatePrefix} onChange={e => setSettings({ ...settings, estimatePrefix: e.target.value })}/></Field>
        </div>
        ...
        <Field label="Notes par défaut"><textarea rows={2} value={settings.defaultNotes} onChange={e => setSettings({ ...settings, defaultNotes: e.target.value })}/></Field>
        <Field label="Instructions de paiement (sur le PDF)"><textarea rows={2} value={settings.paymentInstructions} onChange={e => setSettings({ ...settings, paymentInstructions: e.target.value })} placeholder="Virement Interac à..., chèque à l'ordre de..."/></Field>

        <button className="danger-line" onClick={() => { if (confirm('Remettre les réglages par défaut ? (les invoices et clients restent)')) setSettings(emptySettings) }}>Réinitialiser les réglages</button>
      </div>
    </div>
  </section>
}
